...
def containsSpecies(birb, tree):
    found = False

    if type(tree) is dict:     
        for item in tree:
            if type(tree[item]) is dict or list:
                found = found | containsSpecies(birb, tree[item])
            else:
                if tree[item] is birb:
                    return True
        return found
    elif type(tree) is list:
        for item in tree:
            if type(item) is dict or list:
                found = found | containsSpecies(birb, item)
            else:
                if item is birb:
                    return True
        return found
...
def removeBird(bird, tree):
    if not containsSpecies(bird, tree):
        print("Bird not found.")
        return None
    else:
        return removeBirdHelper(bird, tree)

def removeBirdHelper(birb, tree):
    if type(tree) is dict:  
        found = None   
        for item in tree:
            if type(tree[item]) is dict or type(tree[item]) is list and containsSpecies(birb, tree[item]):
                tree[item] = removeBirdHelper(birb, tree[item])
            else:
                if tree[item] == birb:
                    #print("FOUND YOU: " + tree[item] + "\n\n")
                    found = item
        ...
        while len(tree) > 0:
            ...
            if type(item) is dict or type(item) is list:
                item = removeBirdHelper(birb, item)
                list2.append(item)
            elif item is not birb:
                list2.append(item)
            #else: 
...
def parseRemoved(tree):
    if type(tree) is list and len(tree) is 1:
        for item in tree:
            return parseRemoved(tree[item])
    elif type(tree) is dict and len(tree) == 1:
        for item in tree:
            return parseRemoved(item)
    else:
        return tree

def treeOfList(seen):
    temp = copy.deepcopy(falconTree)
    remove = list(set(species) - set(seen))
    print(remove)
    for item in remove:
        #print("Removing " + item)
        temp = removeBird(item, temp)
    return temp
...
def findSpecies(tree):
    if type(tree) is dict:
        for item in tree:
            if type(tree[item]) is dict or type(tree[item]) is list:
                findSpecies(tree[item])
            else:
                species.append(tree[item])
    elif type(tree) is list:
        while len(tree) > 0:
            item = tree.pop()
            if type(item) is dict or type(item) is list:
...
def countDarwin(tree):
    count = 0
    if type(tree) is dict:
        for item in tree:
            if type(tree[item]) is dict:
                count = count + countDarwin(tree[item]) + item
            elif type(tree[item]) is list:
                if len(tree[item]) == 1:
                    count = count + item
                elif len(tree[item]) == 2:
                    count = count + item + item
            else:
                count = count + item
        return count

""" day2 = ["Falco_fasciinucha", "Falco_subniger", "Falco_moluccensis", "Falco_mexicanus", "Falco_alopex", "Microhierax_caerulescens",  "Micrastur_gilvicollis", "Tyto_alba"]
removedTree = treeOfList(day2)
#print(str(removedTree) + "\n\n")
print(str(pareTree(removedTree)))
for i in range(100):
    removedTree = pareTree(removedTree)
print()
print(removedTree)

print(countDarwin(removedTree))
 """

day = ["Falco_fasciinucha","Falco_sparverius"]
removedTree = treeOfList(day)
for i in range(100):
    removedTree = pareTree(removedTree)
#print(removedTree)
#print(countDarwin(removedTree))